...
@section('content')
@foreach ($content->chunk(3) as $chunk)
<section data-section="section{{ $loop->iteration }}" class="section snap-always snap-start w-full h-full flex flex-col shrink-0 transition bg-light-0 text-black dark:bg-dark-0 dark:text-white" data-theme="media3">
        ...
        <article class="article relative overflow-hidden flex flex-col h-full px-6">
            <div class="article-main relative pt-12 flex-1">
                <!-- <div class="article-asset relative animate animate--fadeInUp mb-6" style="--delay: 300ms">
                    <figure class="article-asset w-full vh-h-landscape aspect-375 bg-cover rounded-xl bg-no-repeat bg-center overflow-hidden" style="background-image: url('assets/images/itaewon/slide_10.jpg');"></figure>
                </div> -->
                <div class="article-desc relative z-20">
                    @foreach ($chunk as $content)
                    @if($loop->odd)
                    <h1 class="article-title vh-text-lg font-semibold mb-6 pr-12  animate animate--fadeInUp" style="--delay: 0ms">
                        {{ $content->textContent }}
                    </h1>
                    @else
