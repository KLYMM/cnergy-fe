<header class="header fixed top-0 inset-x-0 max-w-screen-md mx-auto z-20 h-16" data-header>
    ...
    <div class="flex justify-between items-center " style="padding: 20px 26px 0px 26px">
        <img class="nav-open " src="{{ URL::asset('assets/icons/burger.svg') }}" width="18px" height="18px" alt="bar-icon" onclick="show()" style="cursor: pointer">
...
<div class="nav-main" style="transform: translateX(-450%);">
    <div class="nav-content">
        <div class="nav-header">
            @if ($logo = \Site::api('fe-setting') ?? null)
            <a href="/"> <img class=" dark:img-white" src="{{ $logo['data']['site_logo'] ?? null }}" alt="logo" width="140" height="48" /></a>
            @endif

            <img class="nav-close  dark:img-white" src="{{ URL::asset('assets/icons/icon-close.svg') }}" alt="search-icon" width="25px" height="25px">
        ...
        <ul class="nav-menus">
            ...
            <li class="header-nav-item {{ trim(end($category_tree)['url'] ?? null, '/') == trim($m['url'], '/') || config('site.attributes.object.category.slug') == trim($m['url'], '/') ? 'active' : '' }}">
