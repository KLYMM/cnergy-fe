...
<img src="{{ Src::imgNewsCdn($source, $size, 'webp') ?? Src::mix('img/320x180_no_image.jpg') }}" width="{{ trim(explode('x', $size)[0] ?? '') }}" height="{{ trim(explode('x', $size)[1] ?? '') }}" alt="{{ htmlspecialchars_decode($title ?? 'untitled') }}" {{ (isset($lazy) && $lazy == false) ? '' : 'loading=lazy'}}>
